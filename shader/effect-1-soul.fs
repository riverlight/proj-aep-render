...
uniform mediump float fProgress;

void main()
{
    float maxStrength = 1.5;
    int stage = int((maxStrength - fProgress) * 10.0);
    
    vec4 baseColor = texture2D(colorMap, varyTextCoord);
  
    vec2 soulCoord1 = (varyTextCoord - vec2(0.5)) / fProgress + vec2(0.5);
    vec2 soulCoord2 = (varyTextCoord - vec2(0.5)) / (fProgress * fProgress) + vec2(0.5);
    vec2 soulCoord3 = (varyTextCoord - vec2(0.5)) / (fProgress * fProgress * fProgress) + vec2(0.5);
    vec2 soulCoord4 = (varyTextCoord - vec2(0.5)) / (fProgress * fProgress * fProgress * fProgress) + vec2(0.5);
